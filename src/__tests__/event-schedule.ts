...
import { ScheduledEventTimeline } from '..';

jest.useFakeTimers('legacy');

test('empty event schedule', () => {
  const callback = jest.fn();
  const es = new ScheduledEventTimeline([], callback);
  es.start();
  jest.runAllTimers();
...
test('All events passed', () => {
  ...
  const es = new ScheduledEventTimeline(events, callback);
  es.start();
  jest.runAllTimers();
...
test('Timestamp parsing and sorting', () => {
  ...
  // Ensure "+02:30" is interpreted as 2.5 hours and not as 2.5 minutes
  advanceBy(2 * hour);
